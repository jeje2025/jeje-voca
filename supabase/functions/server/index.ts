...
NOW GENERATE THE JSON ARRAY:`;
    ...
    try {
      ...
      return c.json({ success: false, error: 'Failed to parse JSON response', details: String(parseError) }, 500);
    }
  ...
  try {
    ...
    const { data: vocabulary, error: createError } = await supabase
      ...
      .insert({
        ...
        category: sharedVocab.category,
        level: sharedVocab.level,
        total_words: selectedWordIds ? selectedWordIds.length : sharedVocab.total_words,
      })
      .select()
      .single();
...
app.get("/graveyard", async (c) => {
  try {
    ...
    if (!userId) return unauthorizedResponse(c);
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.from('user_words').select('*').eq('user_id', userId).eq('is_graveyard', true).order('created_at', { ascending: false });
    if (error) throw error;
    return c.json({ words: data.map(hydrateWordResponse) });
  } catch (error) {
    return c.json({ error: String(error) }, 500);
...
app.post("/admin/shared-vocabularies", async (c) => {
  try {
    ...
    const { data: vocabulary, error: vocabError } = await supabase.from('shared_vocabularies').insert({
      id: vocabId, title, category, level: level || difficulty_level || 'Beginner', description: description || '', total_words: words.length,
    }).select().single();
    if (vocabError) throw vocabError;
...
app.get("/admin/categories", async (c) => {
  try {
    ...
    const categories = Array.from(categoriesSet).map(name => ({ id: name.toLowerCase().replace(/\s+/g, '_'), name, enabled: true, order: 0 }));
...
app.post("/admin/categories", async (c) => {
  try {
    ...
    return c.json({ category: { id: name.toLowerCase().replace(/\s+/g, '_'), name, enabled: true, order: 0 } });
  } catch (error) {
    return c.json({ error: String(error) }, 500);
  }
});
...
app.put("/admin/categories", async (c) => {
  try {
    const { id, name, enabled } = await c.req.json();
    return c.json({ category: { id, name: name || id, enabled: enabled !== undefined ? enabled : true, order: 0 } });
  } catch (error) {
    return c.json({ error: String(error) }, 500);
  }
});
...
app.post("/save-vocabulary", async (c) => {
  try {
    const userId = await getUserIdFromAuth(c);
    if (!userId) return unauthorizedResponse(c);
    const { title, category, level, words, vocabularyId } = await c.req.json();
    ...
    const { data: vocabulary, error: vocabError } = await supabase.from('user_vocabularies').insert({
      id: vocabId, user_id: userId, title, category: category || 'My Vocabularies', level: level || 'Beginner', total_words: words.length,
    }).select().single();
    if (vocabError) throw vocabError;
